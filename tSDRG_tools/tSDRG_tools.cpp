...
void tSDRG_regular(vector<MPO>& MPO_chain, vector<double>& J_list, vector<uni10::UniTensor<double> >& VTs, vector<int>& Vs_loc, const int chi, string dis, const int Pdis, const int Jseed, bool save_RG_info, bool& info)
{
    int L = MPO_chain.size();
    double coeff = 1.0;
...
/// tSDRG with regular connect as pyramid
