...
uni10::UniTensor<double> Heisenberg_U1(float spin, double J){
  ...
  bonds.push_back(bdo);
  bonds.push_back(bdo);
  uni10::UniTensor<double> H(bonds, "Heisenberg_U1");
  H.SetRawElem(Heisenberg(spin, J).GetElem());
  return H;
}
...
uni10::UniTensor<double> transverseIsing(float spin, float h, bool isAnti){
  ...
  Id.Identity();
  uni10::Matrix<double> ham = (isAnti) ? uni10::Otimes((double)2*sz, (double)2*sz) : ((double)-1.) * uni10::Otimes((double)2*sz, (double)2*sz); // Otimes(sigma_z, sigma_z);
...
bool load_hamiltonian(uni10::UniTensor<double>& hamiltonian_d, uni10::UniTensor<std::complex<double> >& hamiltonian_c, std::string fname){
  ...
  else if(std::string(name) == "Heisenberg_U1"){
    is_real = true;
    hamiltonian_d = Heisenberg_U1(para1, para2);
  }else{
